<x-back-layout>
    <form action="{{ route('article.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mt-3">
            ...
            <textarea name="content" id="Content" placeholder="Enter Content" class="form-control">{{ old('content') }} </textarea>
            @error('content')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group mt-3">
            <select class="form-select" aria-label="Category" name="categoryid">
                ...
                <option value="{{ $category->id }}" {{ old('categoryid')=="active"?"selected":"" }}>{{ $category->slug}}</option>
                @endforeach
              </select>
              @error('categoryid')
                <span class="text-danger">{{ $message }}</span>
